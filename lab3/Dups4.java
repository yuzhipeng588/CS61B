...
public class Dups4 {
    ...
    static List<String> duplicates(List<String> L) {
        TreeSet<String> result0 = new TreeSet<String>();
        ArrayList<String> result = new ArrayList<String>();
        for (ListIterator<String> p1 = L.listIterator(); p1.hasNext(); ) {
            String x = p1.next();
            if (result0.contains(x)) {
                continue;
            }
            for (ListIterator<String> p2 = L.listIterator(L.size()); p2.previousIndex()!=p1.previousIndex();) {
                String y=p2.previous();
                String z=x;
                if (z.toLowerCase().equals(y.toLowerCase())) {
                    Boolean a =result0.add(x);
                    result.add(x);
                    break;
                }
            }
        }
        /*result0.add(result.last());
        while(!result.isEmpty()){
            result0.add(result.pollLast());
        }*/
        return result;
    }
