...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for(int r=0;r<m.length;r++){
            for(int c=0;c<m[0].length;c++){
                mnew[r][c]=get(mnew,r,c);
            }
        }


        //Zhipeng
        return mnew; //your code here
    }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        if(orientation== Orientation.HORIZONTAL){
            double[][] a=reverse(m);
            return reverse(accumulateVertical(a));
        }
     ...
     */
    //Zhipeng
    public static int getseam(double[]a,int lo,int hi){
        int x=lo;
    ...
    public static int[] findVerticalSeam(double[][] m) {
        //Zhipeng
        double[][] mnew=accumulateVertical(m);
        int[]x=new int[mnew.length];
        if(mnew.length<2){
            return x;
        }
        x[0]=getseam(mnew[1],0,mnew[1].length-1);
        x[1]=x[0];
        for(int r=1;r<mnew.length-1;r++){
            if(x[r]==0){
                x[r+1]=getseam(mnew[r+1],x[r],x[r]+1);
            }
            else if(x[r]==mnew[0].length-1){
                x[r+1]=getseam(mnew[r+1],x[r]-1,x[r]);
            }
            else {
                x[r+1] = getseam(mnew[r+1],x[r]-1,x[r]+1);
            }
        }
        return x;
        //Zhipeng
